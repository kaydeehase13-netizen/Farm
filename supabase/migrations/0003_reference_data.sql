...
-- Tax categories map to Schedule F (Form 1040) line numbers, 2025 revision.
-- Informational labels only — never a legal determination. "Ask your tax
-- professional." (Corrected in 0004_fix_schedule_f_categories.sql for any
-- database that already applied the earlier, off-by-one version of this
-- file — that migration re-applies the same corrected values via upsert,
-- so it's safe to run on top of either version of this one.)
