...
export function createAsset(input: {
  assetType: import("@/types/domain").AssetType; name: string; make?: string; model?: string; year?: number;
  ...
  return mutate((db) => {
    const asset: import("@/types/domain").Asset = {
      id: randomUUID(), farmBusinessId: FARM.id, assetType: input.assetType, name: input.name, make: input.make,
      model: input.model, year: input.year, purchaseDate: input.purchaseDate, purchasePrice: input.purchasePrice,
      placedInServiceDate: input.placedInServiceDate, businessUsePercent: input.businessUsePercent ?? 100,
      usefulLifeYears: input.usefulLifeYears, salvageValue: input.salvageValue ?? 0, notes: input.notes,
      status: "active",
    };
...
export function dashboardSummary(taxYear: number) {
  ...
  const income = txns.filter((t) => t.transactionType === "income" && !isOffBooks(t)).reduce((s, t) => s + t.amount, 0);
  const expenses = txns.filter((t) => t.transactionType === "expense" && !isOffBooks(t)).reduce((s, t) => s + t.amount, 0);
